...
double tend = 50000.0; // make sure to change dtmin if only looking at pump
double dtmin = 1500.0;//250.0; (tend-dt_pr)
...
void RungeKutta2(double t, double h, int iDelta);
void RungeKutta3(double t, double h, int iDelta);
void RungeKutta4(double t, double h, int iDelta);
...
int main(int argc, char* argv[])
{
	...
	int idtmin = atoi(argv[1]);
	dtmin = dtmin+hdt*double(idtmin);
...
//     ab_ddt = new dcomp[Ntotal];
    ...
    if (matricesIn.is_open()){
        ...
        printf("Error Reading data_Matrices \n");
        return 0;
    }
    matricesIn.close();
    ...
    if(deltaIn.is_open()){
        ...
        printf("Error Reading data_Delta \n");
        return 0;
    }    
    ...
    i = 0;
    deltaIn.clear();
    deltaIn.seekg(0,ios::beg);
    while(deltaIn >> in1 >> in2){
        dataDelta[i] = dcomp(in1,in2);
        ++i;  
    }
    deltaIn.close();
    ...
    FILE *aDaOut;
    aDaOut = fopen("data_ProbeMatrices.txt","w");
...
//     reformDiagArrays(); //Comment for tests
    ...
    if(t<tend){
		...
		for (t = t; t<dtmin-dt_pr; t+= h){
            start = get_wall_time();
            
            RungeKutta2(t,h,iDelta);
//             fprintf(deltaOut, "%e\n", abs(Delta));
//             fprintf(data_Delta, "%e\t", real(Delta));
//             fprintf(data_Delta, "%e\n", imag(Delta));
            iDelta += 4;
...
//             fprintf(data_Delta, "%e\n", imag(Delta));
    ...
    if(t<tend){
		...
		for (t = t; t<tend+dtmax; t+= h){
			start = get_wall_time();
            if (iDelta>(num_of_lines-4))
                break;
		
            RungeKutta4(t,h,iDelta);
//             fprintf(deltaOut, "%e\n", abs(Delta));
//             fprintf(data_Delta, "%e\t", real(Delta));
//             fprintf(data_Delta, "%e\n", imag(Delta));
			iDelta += 4;
...
//    fclose(deltaOut);
    ...
    for (i = 0; i<N_pr; i++){ // print final values
        fprintf(aDaOut, "%e\t", real(aDa_pr[i]));
        fprintf(aDaOut, "%e\t", imag(aDa_pr[i]));
        fprintf(aDaOut, "%e\t", real(bDb_pr[i]));
        fprintf(aDaOut, "%e\t", imag(bDb_pr[i]));
        fprintf(aDaOut, "%e\t", real(aDbD_pr[i]));
        fprintf(aDaOut, "%e\n", imag(aDbD_pr[i]));
        fprintf(aDaOut, "%e\t", real(ab_pr[i]));
        fprintf(aDaOut, "%e\n", imag(ab_pr[i]));
    }
    fclose(aDaOut);
...
#pragma omp parallel for schedule(dynamic,1) reduction(+:sumjR,sumjI)
...
//                     (vk_0*uk_pr-uk_0*vk_pr)*(aDbD_c_pr[i]+ab_c_pr[i]));
        ...
        sum1j = 2.0*ky*sum2j/dcomp(Nk[itheta]);
        sumjR += real(sum1j);
        sumjI += imag(sum1j);
    }
    return dcomp(sumjR,sumjI)/dcomp(Ntheta1);
}
